...
unsigned int Distance_VL_H=0;
unsigned int Distance_VL_L=0;
...
void TCS_Get_RGB(int section)
{
	...
	if(((TCS_F.GreenData >= RGB_Threshold_front_up) && (TCS_F.BlueData >= RGB_Threshold_front_up) )/*|| (TCS_F.RedData >= R_White_Threshold)*/)
	{
		Tracer_Stat[SECTION_FRONT] = COLOUR_WHITE;
			...
			((TCS_F.GreenData <  RGB_Threshold_front_up) && (TCS_F.GreenData >  RGB_Threshold_front_low))
														&&
...
void TCS_Get_Colour()
{
	if(Tracer_Stat[SECTION_FRONT] == COLOUR_WHITE)
	{
		Tracer_Stat[FINAL] = COLOUR_WHITE;
	}
	else if(Tracer_Stat[SECTION_FRONT] == COLOUR_GRAY)
	{
		Tracer_Stat[FINAL] = COLOUR_GRAY;
	}
	else if(Tracer_Stat[SECTION_FRONT] == COLOUR_BLACK)
	{
